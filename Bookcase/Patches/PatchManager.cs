...
namespace Bookcase.Patches {
    ...
    class PatchManager {
        ...
        public PatchManager() {
            ...
            BookcaseMod.logger.Info("Creating harmony instance...");
        ...
        private void Load() {
            ...
            assembly.GetTypes()
                .Where(type => type.GetInterfaces().Count(i => i == typePatch) == 0)
                .Where(type => type.GetConstructors().FirstOrDefault(construct => construct.GetParameters().Count() == 0) != null)
                .Select(type => (IGamePatch)type)
                .ToList().ForEach(i => gamePatches.Add(i));
                ...
                if (Array.Exists(type.GetInterfaces(), element => element == typePatch)) {
                    ...
                    foreach(ConstructorInfo constructor in type.GetConstructors()) {
                        ...
                        if (constructor.GetParameters().Length == 0 && constructor.IsPublic) {
                            ...
                            BookcaseMod.logger.Info($"Attempting to construct patch from {type.FullName}.");
                            
                            try {

                                // Try to create an instance of the patch object and load it into our registry.
                                IGamePatch patch = Activator.CreateInstance(type) as IGamePatch;//using as because we aren't checking the cast before making it
                                ...
                                BookcaseMod.logger.Error($"Failed to construct patch {type.FullName} with error {e.Message}.");
                            }
            ...
            BookcaseMod.logger.Info($"Loaded {this.gamePatches.Count} patches!");
        }
        ...
        private void Apply() {
            ...
            foreach (IGamePatch patch in gamePatches) {
                ...
                try {
                    ...
                    BookcaseMod.logger.Info($"Patching {patch.TargetType.ToString()} - {patch.TargetMethod.ToString()}");
                    ...
                    BookcaseMod.logger.Info($"Patch failed: {e.Message}");
                }
            }
        }
